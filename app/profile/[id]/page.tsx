            ...
            <Card className="overflow-hidden border-0 shadow-xl bg-gradient-to-b from-background to-secondary/10">
              ...
              <CardContent className="pt-0 relative">
                <div className="w-24 h-24 rounded-full bg-gradient-to-r from-primary/50 to-purple-500/50 flex items-center justify-center absolute -top-12 left-6 ring-4 ring-background shadow-xl">
                  <span className="font-bold text-3xl">{profile.name.charAt(0)}</span>
                </div>
                    ...
                    <div className="flex">
                      {Array(5)
                        ...
                        .map((_, i) => (
                          <Star
                            key={i}
                            className={`h-5 w-5 ${i < Math.floor(profile.rating) ? "text-yellow-400 fill-current" : "text-muted-foreground"}`}
                          />
                        ))}
                    </div>
                    <span className="font-bold text-lg">{profile.rating}</span>
                    <span className="text-sm text-muted-foreground">({profile.totalRatings} ratings)</span>
                  </div>
          ...
          <div className="lg:col-span-2">
            <Card className="overflow-hidden border-0 shadow-xl bg-gradient-to-b from-background to-secondary/10">
              <CardHeader>
                ...
                <CardDescription>Based on {profile.totalRatings} ratings</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <div key={i} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 h-3 bg-secondary/40 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${(profile.ratingDistribution[5 - rating] / profile.totalRatings) * 100}%` }}
                        ></div>
                      </div>
                      ...
                      <span className="text-xs text-muted-foreground w-16 text-right">
                        {Math.round((profile.ratingDistribution[5 - rating] / profile.totalRatings) * 100)}%
                      </span>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <Card className="bg-secondary/10 border-0">
                    <CardContent className="p-4 text-center">
                      ...
                      <p className="text-3xl font-bold">{profile.ratingDistribution[0]}</p>
                    </CardContent>
                  </Card>
                </div>
              </CardContent>
